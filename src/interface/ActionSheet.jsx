...
const Index = ({
  ...
  const _renderMenuItems = () =>
    menus.map(
      (
        { type: btnType, label, style: btnStyle, textStyle, ...others },
        idx
      ) => (
        <TouchableHighlight
          key={idx}
        ...
        >
          <Text
            style={[
              styles.defaultActionsheetCellText,
  ...
  return _type === "ios" ? (
    ...
    >
      {menus.length ? (
        <View style={[styles.actionsheetMenu]}>{_renderMenuItems()}</View>
      ) : (
        false
      )}
      {actions.length ? (
        <View style={[styles.actionsheetAction]}>{_renderActions()}</View>
      ) : (
        false
      )}
    </Popup>
